...
func TestParseSet(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				action: "set",
				split:  []string{`key`, `"value we qjdjq dfwj "`, "RO"},
			},
			wantSc: SetCommand{
				action: Set,
				key:    "key",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotSc, err := ParseSet(tt.args.split)

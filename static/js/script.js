window.onload = function () {
  ...
  name.addEventListener("keyup", function (e) {
    if (e.keyCode == 13 && name.value != "") {
      ...
      this.value = "";
      if (command == "help") {
        output.innerHTML = "set key value server(optional) - Sets the value to the storage.<br>server > 0 - Save to exact server.<br> server = 0 (default) - Automatic saving to a less loaded server. <br>server = -1 - Direct saving to the database. <br> server = -2 - Saving in all instances.<br> server = -3 - Saving in all instances and DB.  <br><br>get key server(optional) - Gets the value from the storage.<br>server > 0 - Search on a specific server. (speed: fast)<br>server = 0 (default) - Deep search. (speed: slow)<br>server = -1 - DB search. (speed: medium)<br><br>"
        + "new_object name - Creates an object with the specified name.<br>object name set attr value - Sets the value of the object attribute.<br>object name get attr - Gets the value of the object attribute.<br>show_object name - Displays the object as a map.<br>attach dst src - Attaches src object to dst.<br><br>"
        + "history - History of user actions.<br>servers - List of active servers with stats.";
        ...
        + "new_object name - Создает индекс с указанным именем.<br>object name set attr value - Устанавливает значение атрибута object.<br>object name get attr - Получает значение атрибута object.<br>show_object name - Отображает индекс в виде карты.<br>attach dst src - Прикрепляет индекс src к dst.<br><br>"
